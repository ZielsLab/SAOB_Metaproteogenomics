...
out_path="${project_path}/re_binning/np_binning_v2_poly/mappingResults"
...
bowtie2-build ${assembly_path} ${out_path}/bt2/np_assembly.fasta 

# map the Sept experimental sample to the nanpore assembly 
bowtie2 -x ${out_path}/bt2/np_assembly.fasta -1 ${project_path}/00_illumina_for_polishing/illumina_qced/R2Sept2020_R1.qced.fastq -2 ${project_path}/00_illumina_for_polishing/illumina_qced/R2Sept2020_R2.qced.fastq -q --very-sensitive -p 16 -S ${out_path}/R2Sept2020.sam
...
for file in ${ilm_read_path}/R2*_R1.qced.fastq; do
	...
	bowtie2 -x ${out_path}/bt2/np_assembly.fasta -1 ${ilm_read_path}/${name}_R1.qced.fastq -2 ${ilm_read_path}/${name}_R2.qced.fastq -q --very-sensitive -p 16 -S ${out_path}/mappingResults/${name}.sam; 
done 
	
# sort and index files  
for file in ${out_path}/mappingResults/*.sam; do
	name=$(basename $file .sam);
	samtools view -@ 12 -S -b $file > ${out_path}/${name}.bam;
	samtools sort -@ 12 ${out_path}/${name}.bam -o ${out_path}/${name}.sorted.bam;
	samtools index ${out_path}/${name}.sorted.bam;
done 
